...
const NotificationCard = ({ isFixed, onCancel, notification, loading, onClick, icon }: props) => {
  return (
  <Card onClick={onClick} 
  classNames={{ header: "notification-header" }}
  styles={{
    body: {
      display: "flex", 
      gap: isFixed ? 10 : 4, 
  ...
  <div>
    ...
    {!isFixed && <div className='flex items-center gap-3 mt-3'>
      {notification.iconUrl && <Image src={`${process.env.NEXT_PUBLIC_PROFILE_URL2}${notification.iconUrl}`} width={32} height={32} alt='image' className='h-8 w-8 rounded-[100px] object-cover' />}
